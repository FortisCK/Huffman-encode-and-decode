...
class MyDialog:
    def __init__(self):
        ...
        root.title('芜湖，起飞')
        menubar = Menu(root)
        Label(root, text = "这是明文：").grid(row = 1, column = 0)
        Label(root, text = "这是密文：").grid(row = 5, column = 0)
        ...
        tree = Button(Frame1, text = "输出树", command = self.printTree)
